...
- Возможность создавать новую заявку прямо с сайта (запись сразу в Excel, который читает бот).
...
## Роли доступа
- `operator` — просмотр заявок/доски/доступов, может создавать обращения в техподдержку и заявки, но не может администрировать права/роли.
...
2. Узнайте chat_id.
3. Заполните `BotToken` и `ChatIds` в `/opt/repairbot/web/appsettings.json`.
4. Установите `Enabled = true`.
5. Перезапустите `webpanel.service`.
